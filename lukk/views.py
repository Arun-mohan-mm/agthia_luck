...
def check_civil_id(request):
    civil_id = request.GET.get('civil_id', None)
    data = {
        'is_taken': Customer_details.objects.filter(civil_id = civil_id).exists()
    }
...
@login_required
def clr_all_dta_adm(request):
    Customer_details.objects.all().delete()
    return redirect('admin_home')
